...
// efter berättelserna, var serie för sig och i delordning.
const ordning = (a, b) => {
  const sa = serieAv(a), sb = serieAv(b);
  if (!sa !== !sb) return sa ? 1 : -1;
  if (sa) return sa !== sb ? sa.localeCompare(sb) : (delAv(a) ?? 99) - (delAv(b) ?? 99);
  return (a.ar ?? 9999) - (b.ar ?? 9999);
};
