"""
Hittar stjarnorna i fondmalningen och skriver public/stjarnor.json.

    python verktyg/hitta-stjarnor.py [--kontroll fil.png]

Sidan later stjarnorna i malningen glimra i stallet for att lagga ut egna
prickar ovanpa den. Darfor behover den veta var de sitter.

Metod: en stjarna ar en liten punkt som ar tydligt ljusare an sin narmaste
omgivning. Vi jamfor varje pixel med medianen i en 15x15-ruta, tar de lokala
maxima som sticker upp over troskeln och behaller bara de sma omradena.
...
Utdata: [[x, y, s, b, v], ...] dar x,y ar 0..1 i bildens koordinater, s ar
radien i pixlar vid 1600 px bredd, b ar ljushet 0..1 och v ar varmton 0..1
(0 = kallvit, 1 = gulvit).
"""
import json
import sys
...
def hitta(bild):
    ...
    return [s[:5] for s in stjärnor]


def kontrollbild(bild, stjärnor, fil):
...
def main():
    ...
    stjärnor = hitta(bild)
    MÅL.write_text(json.dumps(stjärnor, separators=(",", ":")), encoding="utf-8")
    print(f"{MÅL}: {len(stjärnor)} stjarnor.")
    if "--kontroll" in sys.argv:
        fil = sys.argv[sys.argv.index("--kontroll") + 1]
        kontrollbild(bild, stjärnor, fil)
